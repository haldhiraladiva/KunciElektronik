...
Password password = Password( "1903332073" ); //angak kode PIN 
...
Keypad haldhira = Keypad( makeKeymap(keys), PinBaris,   PinKolom, JmlBaris, JmlKolom ); 
void setup() { 
  ...
  delay(500);
  lcd.clear();
  haldhira.addEventListener(keypadEvent);
}
void loop() {
  pinMode(led, OUTPUT);
   if (a==0) { //state awal
  ...
  haldhira.getKey(); //membaca keypad
}
void keypadEvent(KeypadEvent eKey){ 
  switch (haldhira.getState()){
